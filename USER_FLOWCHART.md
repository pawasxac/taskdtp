# 🔄 Flowchart Alur Pengguna (Eraser.io Syntax)

Dokumen ini berisi kode Diagram-as-Code dalam format **Eraser.io Flowchart Syntax** untuk alur pengguna (*User Flow*) **Daily Coffee Sidoarjo**, lengkap dengan penambahan fitur **Direct Message (DM)** dan limitasinya.

Salin seluruh kode di bawah ini dan tempelkan langsung ke tab **Flowchart** di [Eraser.io](https://www.eraser.io/).

---

```dbml
// Eraser.io - Native Flowchart Diagram-as-Code Syntax
...
// Direct Message Feature Flow
DMMenu [shape: rounded, icon: message-square, label: "Halaman Direct Message (DM)"]
CheckDMLimit [shape: diamond, label: "Pesan Terkirim < 10?"]
SendDM [shape: box, label: "Kirim & Simpan Direct Message"]
ShowSocials [shape: box, label: "Tampilkan Informasi Media Sosial\n(WhatsApp / Instagram / Discord)"]
...
UserDashboard -> DMMenu [label: "Menu: Direct Message"]
UserDashboard -> End [label: "User Keluar Website"]
...
// Direct Message Path
DMMenu -> CheckDMLimit
CheckDMLimit -> SendDM [label: "Ya"]
CheckDMLimit -> ShowSocials [label: "Tidak"]
SendDM -> DMMenu
ShowSocials -> DMMenu
DMMenu -> End
```
